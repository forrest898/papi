...
int PAPI_sample_init(int Evenset, int EventCode, long long sample_type,
			long long sample_period, struct PAPI_sample_info *sample_info,
			PAPI_overflow_handler_t handler)
{
	...
	if(!(ESI->state) & PAPI_SAMPLING) ) {
		if(handler == NULL) {
			OVFDBG("Null handler\n");
			papi_return( PAPI_EINVAL );
		}
		/* Some kind of check for read_format and sample_type */
	}
